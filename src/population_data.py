...
df = pd.read_csv(
    os.path.join(
        basepath,
        "csv_Demographie_100m_Gitter/Bevoelkerung100M.csv",
    ),
    sep=";",
    encoding="latin_1",
)
...
geodf_pop = geodf_pop.to_crs(
    crs
)

# clip geomtry by SH boundaries
sh_poly = cascaded_union(
    polygons.to_crs(
        crs
    )
)
...
for age in geodf_sh.columns[
    2:11
]:
    arr = rasterize(
        zip(
            geodf_sh.geometry.apply(
                mapping
            ).values,
            geodf_sh[
                age
...
ds = xr.combine_by_coords(
    [
        i.to_dataset()
        for i in pop_by_ageclass.values()
    ]
)
