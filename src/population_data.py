...
sh_shapes = gpd.read_file(os.path.join(basepath, "shapefile_sh", "schleswig-holstein.shp") ,crs="4326")
polygons = sh_shapes.geometry


# Read ZENSUS data
df = pd.read_csv(
    os.path.join(basepath, "csv_Demographie_100m_Gitter/Bevoelkerung100M.csv"),
    sep=";",
    encoding="latin_1",
)
...
grid_path = os.path.join(
    basepath,
    "DE_Grid_ETRS89_LAEA_100m/geogitter")
...
geodf_sh = geodf_sh.fillna(0) # need to replace NaN for rasterize to work properly
pop_by_ageclass = {}
for age in geodf_sh.columns[2:11]:
    arr = rasterize(
        zip(
            geodf_sh.geometry.apply(mapping).values,
            geodf_sh[age],
        ),
...
ds.to_netcdf(
    os.path.join(
         "/home/admin/nextcloud-znes/KlimaSchiff/population_data", "population.nc"
